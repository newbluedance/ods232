...
<body>
<div class="box2" panelTitle="查询项目" id="searchPanel">
    <form action="<%=request.getContextPath()%>/project/query" id="queryForm" method="post">
        <input type="hidden" id="parentId" name="parentId" value="1"/>
        <table>
            <tr>
                <td>项目名称：</td>
                <td>
                    <label for="searchInput"><input type="text" id="searchInput" name="projectName" /></label>
                    <label>
...
function initComplete(){
    ...
    grid = $("#dataBasic").quiGrid({
        columns:[
            { display: '项目名称', name: 'projectName',     align: 'left', width: "20%"},
            { display: '服务器地址', name: 'serverUrl', 	 align: 'left', width: "20%"},
            { display: '是否激活', name: 'isActive', align: 'left', width: "20%"},
        ...
        url: '<%=request.getContextPath()%>/project/query', sortName: 'F_PROJECT_ID',rownumbers:true,checkbox:true,
...
function onDelete(rowid,rowidx){
    ...
    top.Dialog.confirm("确定要删除该记录吗？",function(){
        //删除记录
        $.post("<%=request.getContextPath()%>/project/delete",
                {"ids":rowid},
                function(result){
...
function refresh(isUpdate){
    if(!isUpdate){
        //重置排序
        grid.options.sortName='F_PROJECT_ID';
