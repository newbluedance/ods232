...
<body>
	<div id='mainDiv' class="box1" whiteBg="true">
		<form action="<%=request.getContextPath()%>/proBacInfo/updateProBacInfo" id="updateForm" method="post"
			...
			<table class="tableStyle" formMode="view">
				...
				<tr>
					<td width="25%">hs编码：</td>
					<td width="25%"><input id="hsCode" name="hsCode" type="text" value="${proBacInfo.hsCode}" /></td>
					<td width="25%">产品名称：</td>
					<td width="25%"><input id="proName" name="proName" type="text" value="${proBacInfo.proName}" /></td>
				</tr>
				<tr>
					<td>产品市场名称：</td>
					<td><input id="promName" name="promName" type="text" value="${proBacInfo.promName}" /></td>
					<td>产品类型：</td>
					<td>
						<input id="classId" name="classId" type="hidden" value="${proBacInfo.classId}" />
						<div class="selectTree" selectedValue="${proBacInfo.classId}" url="<%=request.getContextPath()%>/proClass/proClassList" id="selectTree1"></div>
					</td>
				</tr>
				<tr>
					<td>制作商：</td>
					<td><input id="maker" name="maker" type="text" value="${proBacInfo.maker}" /></td>
					<td>产地：</td>
					<td><input id="address" name="address" type="text" value="${proBacInfo.address}" /></td>
				</tr>
				<tr>
					<td>品牌：</td>
					<td><input id="brand" name="brand" type="text" value="${proBacInfo.brand}" /></td>
					<td>产品录入时间：</td>
					<td><input id="createDate" name="createDate" value="${proBacInfo.createDate}" type="text"
						class="validate[required]" class="dateIcon" onfocus="WdatePicker({skin:themeColor,maxDate:'%y-%M-%d'})" /> <span
						class="star">*</span></td>
				</tr>
			</table>
			<table  class="tableStyle" formMode="view">
		...
		$("#selectTree1").bind(
				...
				function() {
					...
					//	top.Dialog.alert("请选择节点");
						...
						if($(this).attr("relValue")=='${proBacInfo.classId}'){
							initFieldBox();
						}else{
						//初始化属性区域
