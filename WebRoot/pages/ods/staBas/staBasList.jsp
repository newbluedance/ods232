...
<body>
	<div class="box2" panelTitle="查询" id="searchPanel">
	    <form action="<%=request.getContextPath()%>/staBas/staBasList" id="queryForm" method="post">
	        <table>
	            <tr>
	...
	<script type="text/javascript">
	    ...
	    function initComplete(){
	        g = $("#maingrid").quiGrid({
	            columns: [
	            	{ display: '产品编号', name: 'proId',     align: 'center', width: "10%"},
	            	{ display: 'hs编码', name: 'hsCode',     align: 'center', width: "10%"},
	                { display: '产品名称', name: 'proName',     align: 'center', width: "10%"},
	                 { display: '产品类别', name: 'className',   align: 'center', width: "10%" },
	                { display: '录入时间', name: 'createDate',   align: 'center', width: "10%"},
	                { display: '制作商', name: 'maker',   align: 'center', width: "10%"},
	                { display: '产品市场名称 ', name: 'promName', align: 'center', width: "10%"},	 
	                { display: '审核状态', name:'stateS', align: 'center', width: "10%",type:'stateS'},
	                	/* render: function (rowdata, rowindex, value, column){
	                		...
	                		if(rowdata.stateS == '0'){
	                			str1="已申请";
	                		}else if(rowdata.stateS == '1'){
	                			str2="已提交";
	                		}else if(rowdata.stateS=='2'){
	                			str3="审核通过";
	                		}else if(rowdata.stateS=='3'){
	                			str4="退回";
	                		}else if(rowdata.stateS==null){
	                			str5="未新增";
	                		}
	                ...
	                { display: '操作', isAllowHide: false, align: 'center', width: "20%",
	                    ...
	                    render: function (rowdata, rowindex, value, column){
	                    	...
	                    	var str3="";
	                    	var str4="";
	                    	if(rowdata.stateS == '0' || rowdata.stateS=='3'){
	            ...
	            url: '<%=request.getContextPath()%>/staBas/queryproBacList',
	            pageSize: 10,
	            sortName: '',
	            rownumbers:true,
	            checkbox:false,
	        ...
	        $.quiDefaults.Grid.formatters['stateS'] = function (value, column) {
	        	if(value == '0'){
        			return "已申请";
        		}else if(value == '1'){
        			return "已提交";
	        ...
	        $("#searchPanel").bind("stateChange", function (e, states) {
	            g.resetHeight();
	        });
	    }
	    //添加
	    function toAdd(proId) {
	   	window.location.href="<%=request.getContextPath()%>/staBas/toAdd?proId="+proId;
	    }
		//查看
	    function toDetail(proId) {
	       	 window.location.href="<%=request.getContextPath()%>/staBas/toDetail?proId="+proId;
	    }
		    //审核通过后查看 
	    ...
	    function toTijiao(proId) {
	    	if(confirm('确定要提交该记录吗？')){
	        	 window.location.href="<%=request.getContextPath()%>/staBas/toTijiao?proId="+proId ;	
	        }    	
	    }
	     //删除
	    function del(staId) {
	    	top.Dialog.confirm("确定要删除该记录吗？",function(){
	        	 window.location.href="<%=request.getContextPath()%>/staBas/delStaBas?staId="+staId ;
	        });
	    }
	     //重置查询
