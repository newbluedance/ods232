...
<body>
	...
	<script type="text/javascript">
	    ...
	    function initComplete(){
	        g = $("#maingrid").quiGrid({
	            columns: [
	            	{ display: '产品编号', name: 'proId',     align: 'center', width: "15%", isSort:false},
	            	{ display: 'hs编码', name: 'hsCode',     align: 'center', width: "15%", isSort:false},
	                { display: '产品名称', name: 'proName',     align: 'center', width: "10%", isSort:false},
	                 { display: '产品类别', name: 'classId',   align: 'center', width: "10%" , isSort:false},
	                ...
	                { display: '操作', isAllowHide: false, align: 'center', width: "20%",
	                    ...
	                    render: function (rowdata, rowindex, value, column){
	                    	return '<div class="padding_top4 padding_left5">'
	                    			+'<input class="button2" onclick="toDetail(\'' + rowdata.proId + '\')" type="button" width="45px" value="查看"/>'
	                                + '</div>';
	                    	}
	                    }
	            ],
	            url: '<%=request.getContextPath()%>/proStaBas/proStaBasList',
	            pageSize: 10,
	            sortName: '',
	            rownumbers:true,
	            checkbox:false,
	    ...
	    function toDetail(proId) {
	    	window.location.href="<%=request.getContextPath()%>/proStaBas/toDetail?proId="+proId;
	    }
	     //重置查询
