...
<body>
	<div id='handlerDiv' class="box1" whiteBg="true">
			...
			<tr>
					<c:set var="n" value="0"></c:set> 
			     	<c:forEach items="${list}" var="s" varStatus="status">
					    ...
					    <td  style="text-align:right; font-weight:bold; height:50px;width:'10%';">
						     ${s.posClass}
						</td>
					     </c:if>
		...
		<table class="tableStyle" formMode="view">
			...
			<tr>
				<td width="25%">审核人：</td>
				<td width="25%">${proBacInfo.checkNameH}</td>
				<td width="25%">审核意见：</td>
				<td width="25%">${proBacInfo.checkConentH}</td>
			</tr>
			<tr>
				<td width="25%">审核状态：</td>
				<td width="25%"><c:if test="${proBacInfo.stateH == 2}">
							审核通过
						</c:if> <c:if test="${proBacInfo.stateH == 3}">
							退回
						</c:if></td>
				<td width="25%">审核时间：</td>
				<td width="25%">${proBacInfo.checkDateH}</td>
			</tr>
		</table>
...
<script type="text/javascript">
		...
		function(fieldList) { //回调函数
			...
			for ( var n in fieldList) {
				...
				var fieldValue = (field.fieldValue == null) ? ""
						: field.fieldValue;
				exc_field_node = exc_field_node + "<td width='25%'>"
						+ field.fieldKey + "：</td>" + "<td width='25%'>"
						+ fieldValue + "</td>";
				flag = (parseInt(n) + 1) % 2;
				if (flag == 0) {
