package com.ninetowns.pulgins.mybatis.plugin;
...
import com.ninetowns.pulgins.jdbc.dialet.Dialect;
...
//                    return ivk.proceed();
                    ...
                    if (parameterObject instanceof PageView) { // 参数就是Pager实体
...
//						pager.setRowCount(count);
     ...
     * @return
     */
    private String generatePagesSql(String sql, PageView pageView) {
        if (pageView != null && dialectObject != null) {
            //pageNow默认是从1，而已数据库是从0开始计算的．所以(page.getPageNow()-1)
            int pageNow = pageView.getPager().getPageNo();
            return dialectObject.getLimitString(sql, (pageNow <= 0 ? 0 : pageNow - 1)
                    * pageView.getPager().getPageSize(), pageView.getPager().getPageSize());
        }
        return sql;
    }
